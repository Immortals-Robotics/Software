...
public:
	...
	{
		if ( ( fabs ( NormalizeAngle ( bState->velocity.direction - AngleWith ( bState->Position , Vec2 ( rState->Position.X + BAR * cosDeg ( rState->Angle ) , rState->Position.Y + BAR * sinDeg ( rState->Angle ) ) ) ) ) < 40 ) && ( fabs(NormalizeAngle(180+bState->velocity.direction-rState->Angle)) < 100 ) && ( bState->velocity.magnitude > 50 ) )//&&(abs(ball.vel_angle-90)>0.01)&&(abs(ball.vel_angle+90)>0.01)&&(abs(ball.vel_angle-180)>0.01)&&(abs(ball.vel_angle+180)>0.01))
		{
			hys = 30;
			return true;
		}
