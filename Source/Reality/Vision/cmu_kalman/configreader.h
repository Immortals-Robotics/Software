...
private:
  ...
  std::vector<ConfigFile> filereaders;

  int FindFile(const char *const fname) {
  ...
  bool ReadFile(const char *const _fname);

  const char *path;

public:
  std::vector<PDataMap> datamap;
  ...
  DataMap *FindDataMapping(const char * const _fname, const char *const _dataname, int _type);
