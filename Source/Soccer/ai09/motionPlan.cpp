...
TVec3 Robot::MotionPlan(RobotState state, RobotState target, float speed, bool accurate, TVec3 *cmd,
...
//    LocalVel.Y = (LocalVel.Y) / 45.0f;
    ...
    } else
    {
        ...
        {
            if (sgn(ans.Y) == 0)
                ans.Y = max(fabs(oldAns[state.vision_id].Y) - max_dec.Y, fabs(tmp_max_speed.Y)) *
                        sgn(oldAns[state.vision_id].Y);
            else
